...
public class CalcRevenue extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			int i = 0;
			PreparedStatement ps = con.prepareStatement("SELECT SUM(invoice.grand_total) AS gtotal, user_detail.city from invoice, user_detail WHERE invoice.cid = user_detail.userid AND invoice.date >= '" + fdt + "' AND invoice.date <= '" + tdt + "' GROUP BY user_detail.city");
